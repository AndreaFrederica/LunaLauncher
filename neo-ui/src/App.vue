...
<q-layout view="hHh Lpr fFf" class="bg-grey-10 text-white">
 <q-header><q-toolbar><q-toolbar-title>Luna Neo UI</q-toolbar-title><q-btn flat label="刷新" :disable="busy" @click="refresh" /></q-toolbar></q-header>
 <q-drawer show-if-above :width="210" class="bg-grey-9 text-white"><q-list padding>
  <q-item v-for="item in pages" :key="item.id" clickable :active="page===item.id" @click="navigate(item.id)"><q-item-section avatar><q-icon :name="item.icon" /></q-item-section><q-item-section>{{ item.label }}</q-item-section></q-item>
 </q-list></q-drawer>
 ...
 <div v-if="page==='instances'">
  ...
  <p v-if="!instances.length && !busy">还没有实例，点击添加实例创建或导入。</p>
 ...
 <div v-if="page==='create'" class="form"><h5>添加实例</h5>
  ...
  <template v-if="createMode==='vanilla'"><q-input dark outlined v-model="creation.version" label="Minecraft 版本，例如 1.21.1" class="q-mb-md"/><q-select dark outlined v-model="creation.loader" :options="['','fabric','forge','neoforge','quilt']" label="Mod Loader（可选）" class="q-mb-md"/><q-input v-if="creation.loader" dark outlined v-model="creation.loaderVersion" label="Loader 版本" class="q-mb-md"/></template>
  <q-input v-else dark outlined v-model="creation.source" label="整合包本地完整路径或下载 URL" class="q-mb-md"/>
  <q-btn color="primary" label="创建实例" :disable="busy || !creation.name || (createMode==='vanilla' ? !creation.version : !creation.source)" @click="create"/>
 </div>
 ...
 </div><q-input dark outlined autogrow v-model="notes" label="实例备注"/><q-btn class="q-mt-md" label="保存备注" :disable="busy" @click="run('instance.set-notes',{instance:selected.id,notes})"/></div>
...
async function refresh(){try{if(page.value==='accounts')accounts.value=await call('account.list');else if(page.value==='settings')settings.value=await call('settings.list',settingScope());else instances.value=await call('instance.list');}catch(e){error.value=message(e);}}
...
async function openInstance(i:any){selected.value=i;page.value='manage';notes.value='';try{const info=await call('instance.info',{instance:i.id});notes.value=info.notes || '';}catch(e){error.value=message(e);}}

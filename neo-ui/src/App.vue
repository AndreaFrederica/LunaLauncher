...
<q-layout view="hHh Lpr fFf" class="neo-shell text-white">
 <q-header class="neo-header"><q-toolbar class="neo-toolbar"><q-btn aria-label="打开导航" flat round icon="menu" class="lt-md" @click="drawerOpen=!drawerOpen"/><q-avatar square size="34px" class="brand-mark"><q-icon name="auto_awesome"/></q-avatar><q-toolbar-title class="brand-title">Luna <span>Neo</span><small>{{ buildLabel }}</small></q-toolbar-title><div class="header-status"><q-icon name="circle" size="9px" :color="connected?'positive':'grey'"/> {{ connected ? '已连接启动器' : '未连接启动器' }}</div><q-btn aria-label="刷新" flat round icon="refresh" :disable="busy" @click="refresh" /></q-toolbar></q-header>
 <q-drawer v-model="drawerOpen" :behavior="$q.screen.lt.md ? 'mobile' : 'desktop'" :width="248" class="neo-drawer">
  ...
  <div class="drawer-label">工作区</div><q-list class="nav-list">
   <q-item v-for="item in pages" :key="item.id" clickable :active="page===item.id" active-class="nav-active" @click="navigate(item.id);closeMobileNav()"><q-item-section avatar><q-icon :name="item.icon"/></q-item-section><q-item-section>{{ item.label }}</q-item-section></q-item>
   <q-item v-if="selected" clickable :active="page==='manage'" active-class="nav-active" @click="navigate('manage');closeMobileNav()"><q-item-section avatar><q-icon name="tune"/></q-item-section><q-item-section>实例管理</q-item-section></q-item>
  </q-list><div class="drawer-footer"><q-icon name="info"/> Neo UI {{ buildLabel }}</div>
 </q-drawer>
 <q-page-container><q-page padding class="neo-page">
 <q-tabs v-if="openedInstances.length" :model-value="page==='manage'?selected?.id:''" align="left" outside-arrows mobile-arrows class="q-mb-lg"><q-tab v-for="i in openedInstances" :key="i.id" :name="i.id" :label="i.name" icon="sports_esports" @click="openInstance(i)"/></q-tabs>
 <q-banner v-if="launchQueue.length" class="bg-grey-9 q-mb-md">等待启动：{{ launchQueue.map(i=>i.name).join('、') }}<template #action><q-btn flat label="清空队列" @click="launchQueue=[]"/></template></q-banner>
 ...
 <div v-if="page==='instances'">
  ...
  <div class="library-toolbar"><q-input dark borderless v-model="search" placeholder="搜索实例或分组" class="library-search"><template #prepend><q-icon name="search"/></template></q-input><div class="library-count"><strong>{{ instances.length }}</strong> 个实例</div></div>
  <p v-if="!instances.length && !busy && !error">还没有实例，点击添加实例创建或导入。</p>
  <div class="row q-col-gutter-lg"><div class="col-12 col-sm-6 col-lg-4" v-for="i in instances.filter(x => [x.name,x.id,x.group].join(' ').toLowerCase().includes(search.toLowerCase()))" :key="i.id">
   <q-card class="instance-card"><div class="instance-art"><img v-if="icons[i.id]" :src="icons[i.id]" :alt="i.name+' 图标'" width="64" height="64" style="object-fit:contain"/><q-icon v-else name="sports_esports" size="34px"/><span>{{ i.running ? '运行中' : '就绪' }}</span></div><q-card-section><div class="instance-name">{{ i.name }}</div><div class="instance-meta"><q-icon name="folder"/> {{ i.group || '未分组' }}</div><div class="instance-id">{{ i.id }}</div></q-card-section>
   <q-card-actions class="instance-actions"><q-btn unelevated color="primary" icon="play_arrow" :label="busy ? '加入启动队列' : '启动'" @click="launch(i)" /><q-btn aria-label="管理实例" flat round icon="more_horiz" @click="openInstance(i)" /></q-card-actions></q-card>
  </div></div>
 </div>
 <div v-if="page==='create'" class="form"><h5>添加实例</h5>
  <q-tabs v-model="createMode"><q-tab name="vanilla" label="Minecraft"/><q-tab name="import" label="导入整合包"/><q-tab name="catalog" label="下载整合包"/></q-tabs>
  <CatalogBrowser v-if="createMode==='catalog'" packs :busy="busy" :execute="catalogExecute" @installed="navigate('instances')"/>
  <template v-else>
  ...
  <q-btn color="primary" label="创建实例" :disable="busy || !creation.name || (createMode==='vanilla' ? (!creation.version || (!!creation.loader && !creation.loaderVersion)) : !creation.source)" @click="create"/></template>
 </div>
 <div v-if="page==='manage' && selected"><h5>{{ selected.name }} · 实例管理</h5><div class="row q-gutter-sm q-mb-lg">
  ...
  <q-btn label="更换图标" :disable="busy" @click="changeIcon"/><q-btn label="打开实例目录" :disable="busy" @click="run('instance.open-folder',{instance:selected.id})"/><q-btn label="实例设置" @click="scope='instance'; page='settings'; refresh()"/>
 ...
 <q-expansion-item v-if="['mods','resourcepacks','shaderpacks','datapacks'].includes(resourceKind)" label="下载模组 / 资源" icon="download" class="q-my-md"><CatalogBrowser :instance="selected.id" :kind="resourceKind" :minecraft-version="instanceDetails?.minecraftVersion" :busy="busy" :execute="catalogExecute" @installed="loadResources"/></q-expansion-item>
 ...
 <JavaPanel :scope="settingScope()" :settings="settings" :busy="busy" :execute="catalogExecute" @changed="refresh"/>
 <SettingsPanel :settings="settings" :busy="busy" @save="(key,value)=>run('settings.set',{...settingScope(),key,value})" @reset="key=>run('settings.reset',{...settingScope(),key})"/>
 </div>
...
import JavaPanel from './components/JavaPanel.vue';
import CatalogBrowser from './components/CatalogBrowser.vue';
import SettingsPanel from './components/SettingsPanel.vue';
import { useQuasar } from 'quasar';
import { launcher } from './boot/launcher-api';
const buildLabel = '2026-09-20.1';
const quasar=useQuasar(),connected=ref(false),icons=ref<Record<string,string>>({});
function closeMobileNav(){if(quasar.screen.lt.md)drawerOpen.value=false;}
async function loadIcons(){for(const i of instances.value){if(busy.value)break;try{const result=await call('instance.icon',{instance:i.id});icons.value[i.id]=result.dataUrl;}catch{ /* Older backends retain the fallback icon. */ }}}
...
const openedInstances=ref<any[]>([]),launchQueue=ref<any[]>([]);
const detailCache=new Map<string,any>();
let launchingId='',preparingLaunch=false;
const sessions=new Map<string,{subscription:string;lines:string[];state:string}>();
let consoleSubscription='',consoleInstance='',launchInFlight=false;
async function appendConsole(text:string,instanceId=selected.value?.id){
 if(instanceId!==selected.value?.id){const saved=sessions.get(instanceId);if(saved){saved.lines.push(text);saved.lines.splice(0,Math.max(0,saved.lines.length-3000));}return;}
...
function receiveStream(batch:any){
 const entry=[...sessions.entries()].find(([,v])=>v.subscription===batch.subscriptionId);
 if(!entry)return;
 if(entry[0]!==selected.value?.id){for(const event of batch.events||[]){if(event.kind==='console.line')entry[1].lines.push(event.text);if(event.kind==='instance.state')entry[1].state=event.running?'客户端运行中':'客户端已停止';}entry[1].lines.splice(0,Math.max(0,entry[1].lines.length-3000));return;}
...
async function connectConsole(i:any){
 ...
 try{
  const existing=sessions.get(i.id);if(existing?.subscription){consoleSubscription=existing.subscription;consoleInstance=i.id;consoleLines.value=existing.lines;consoleState.value=existing.state;return;}
  consoleSubscription='';consoleInstance=i.id;consoleLines.value=[];
  const sub=await call('instance.console.subscribe',{instance:i.id});consoleSubscription=sub.subscriptionId;sessions.set(i.id,{subscription:consoleSubscription,lines:consoleLines.value,state:'控制台已连接'});
  consoleState.value='控制台已连接';
 }catch(e){error.value=message(e);consoleState.value='连接失败';}
}
async function launch(i:any){
 if(busy.value){if(i.id!==launchingId&&!launchQueue.value.some(x=>x.id===i.id))launchQueue.value.push(i);return;}
 busy.value=true;launchingId=i.id;preparingLaunch=true;
 await openInstance(i,true);logMode.value='live';await connectConsole(i);preparingLaunch=false;
 if(!consoleSubscription){busy.value=false;launchingId='';return;}
 ...
 }catch(e){error.value=message(e);void appendConsole('[启动失败] '+error.value,launchingId);}
 finally{busy.value=false;launchInFlight=false;launchingId='';device.value=null;status.value='';}
}
...
watch(busy,async value=>{if(!value && launchQueue.value.length){await nextTick();if(!busy.value){const next=launchQueue.value.shift();if(next)void launch(next);}}});
...
async function catalogExecute(op:string,p:Record<string,unknown>){if(busy.value)throw new Error('请等待当前操作完成');busy.value=true;try{return await call(op,p);}finally{busy.value=false;status.value='';}}
async function call(op:string,p:Record<string,unknown>={}){const r=await launcher.execute<any>(op,p);if(!r.ok)throw new Error(r.error||'操作失败');connected.value=true;return r.data;}
...
async function refresh(){error.value='';try{if(page.value==='accounts')accounts.value=await call('account.list');else if(page.value==='settings')settings.value=await call('settings.list',settingScope());else if(page.value==='manage'){await loadResources();await loadComponents();await loadLogFiles();}else {instances.value=await call('instance.list');await loadIcons();}}catch(e){error.value=message(e);}}
async function navigate(id:string){if(preparingLaunch)return;page.value=id;error.value='';notice.value='';if(id==='settings')scope.value='launcher';if(!busy.value)await refresh();}
...
async function openInstance(i:any,forceRead=false){
 if(preparingLaunch&&!forceRead)return;
 if(selected.value)detailCache.set(selected.value.id,{info:instanceDetails.value,notes:notes.value,components:components.value,resources:resources.value,kind:resourceKind.value,logFiles:logFiles.value});
 if(!openedInstances.value.some(x=>x.id===i.id))openedInstances.value.push(i);
 if(selected.value?.id!==i.id){
  const old=sessions.get(selected.value?.id);if(old){old.lines=consoleLines.value;old.state=consoleState.value;}
  const saved=sessions.get(i.id);consoleSubscription=saved?.subscription||'';consoleInstance=i.id;consoleLines.value=saved?.lines||[];consoleState.value=saved?.state||'未连接';logFile.value='';fileLog.value='';logFiles.value=[];
 }
 selected.value=i;page.value='manage';notes.value='';error.value='';instanceDetails.value=null;
 if(busy.value&&!forceRead){const cached=detailCache.get(i.id);instanceDetails.value=cached?.info;notes.value=cached?.notes||'';components.value=cached?.components||[];resources.value=cached?.resources||[];resourceKind.value=cached?.kind||'mods';logFiles.value=cached?.logFiles||[];notice.value='正在处理其他任务；显示缓存内容，任务完成后可刷新。';return;}
...
async function changeIcon(){const path=await ask('图标文件完整路径（PNG / ICO 等）');if(path===null||!path.trim())return;await run('instance.set-icon',{instance:selected.value.id,path:path.trim()});if(!error.value){const icon=await call('instance.icon',{instance:selected.value.id});icons.value[selected.value.id]=icon.dataUrl;}}
...
onMounted(async()=>{try{unlistenStream=await launcher.onStream(receiveStream);unlistenExit=await launcher.onExit(()=>{connected.value=false;sessions.clear();consoleSubscription='';consoleState.value='后端已退出';error.value='启动器后端已退出，请刷新重连。';});unlisten=await launcher.onEvent(async(raw:any)=>{if(raw.kind==='input'){const opts=raw.choices?.map((x:any,n:number)=>({label:typeof x==='string'?x:(x.name||x.label||JSON.stringify(x)),value:n}));const v=await ask(raw.prompt,'',false,opts||null,raw.secret);try{await launcher.respond(v===null?{interactionId:raw.interactionId,cancel:true}:{interactionId:raw.interactionId,value:v});}catch(e){error.value=message(e);}}else if(raw.kind==='device_code'){device.value=raw.data || raw;}else if(raw.kind==='status'){if(launchInFlight)void appendConsole('[启动] '+(raw.message || raw.data?.message || ''),launchingId);status.value=raw.message || raw.data?.message || (typeof raw.data==='string'?raw.data:'正在执行');}else if(raw.kind==='task'){status.value=raw.data?.status||raw.data?.state||'正在执行';}});await refresh();}catch(e){error.value=message(e);}});
